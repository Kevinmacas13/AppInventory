...
import { Config } from "@inventario/core/src/shared/config";
...
const apiCall = tool({
  ...
  args: z.object({
    ...
    contentType: z
      .string()
      .optional()
      .describe("Tipo de contenido HTTP a utilizar"),
...
const getOpenApiDocs = tool({
  ...
  async run() {
    ...
    if (!response.ok)
      throw new Error("No se pudo obtener la documentación OpenAPI");
    return response.text();
  },
});
